...
MIN_SPACE=$((50 * 1024 * 1024))
AVAILABLE_SPACE=$(df --output=avail "$DOCKER_DIR" | tail -n 1)

if (( AVAILABLE_SPACE < MIN_SPACE )); then
    echo "Warning: The available space in the Docker image storage directory ($DOCKER_DIR) is less than 50GB. Please free up some space or set a new image storage directory."
    exit 1
fi

sudo apt install speedtest-cli -y || true
